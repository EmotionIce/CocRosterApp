...
(function (root, factory) {
  ...
  const REGULAR_CONTEXT_MODES = ["off", "explain", "assist", "automatic"];
  ...
  const sanitizeRegularAttackContext = (raw) => {
    const value = raw && typeof raw === "object" ? raw : {};
  ...
  const isValidatedForcedHardTarget = (attack) => !!(
    attack &&
    attack.formEligible &&
  ...
  const analyzeRegularContext = (eventsRaw, totalsRaw) => {
    ...
    for (const eventRaw of events) {
      ...
      if (first.mirrorResolved && first.targetResolved && first.mirrorStarsBefore < 3) {
        mirrorEvaluableWars++;
        if (!first.hitMirror) mirrorViolationWars++;
      }
      if (
        ...
        if (first.ownAttackOrdinal / context.maxOwnAttacks >= 0.7) {
          lateLowTownHallWars++;
          if (isValidatedForcedHardTarget(first)) lateForcedHardWars++;
        }
      }
    }
  ...
  const formatRegularAttackContextLines = (contextRaw) => {
    ...
    return context.attacks.map((attack, index) => {
      ...
      return "Attack " + attackNumber + ": " + (attacker ? attacker + " · " : "") + mirror + " · target " +
        targetParts.filter(Boolean).join(" ") + " · " + timing + " · " + attack.stars + "★ " + attack.destruction + "% · " + status;
    });
  };
  ...
  const cwlSeasonStartAt = (seasonRaw) => {
  ...
  const mergeEvidenceSources = (primaryRaw, secondaryRaw, limitRaw, kindRaw) => {
    ...
    const idsFor = (eventRaw) => {
      const event = eventRaw && typeof eventRaw === "object" ? eventRaw : {};
      return Array.from(new Set([event.id].concat(Array.isArray(event.legacyIds) ? event.legacyIds : [])
        .map((value) => toText(value).trim())
        .filter(Boolean)));
    };
    const sameEvent = (left, right) => {
      if (kind === "regular") {
        const leftClan = normalizeTag(left && left.clanTag);
        const rightClan = normalizeTag(right && right.clanTag);
    ...
    for (const fallback of Array.isArray(secondary.events) ? secondary.events : []) {
      ...
      const preferredAt = parseMs(preferred && preferred.at);
      const fallbackAt = parseMs(fallback && fallback.at);
      const preferredFinalizedAt = parseMs(preferred && preferred.finalizedAt);
      const fallbackFinalizedAt = parseMs(fallback && fallback.finalizedAt);
      events[index] = Object.assign({}, fallback, preferred, {
        ...
        seasonStartedAt: toText(preferred && preferred.seasonStartedAt || fallback && fallback.seasonStartedAt).trim(),
        clanTag: normalizeTag(preferred && preferred.clanTag || fallback && fallback.clanTag),
        stats: kind === "cwl" ? moreCompleteCwlStats(preferred && preferred.stats, fallback && fallback.stats) : preferred.stats,
        context: kind === "regular" ? (preferred && preferred.context || fallback && fallback.context) : undefined,
        legacyIds: Array.from(new Set(idsFor(preferred).concat(idsFor(fallback))))
          .filter((id) => id !== toText(preferred && preferred.id).trim()),
      });
    }
    ...
    totals.warCount = kind === "cwl"
      ? events.reduce((sum, event) => sum + toInt(event && event.stats && event.stats.warCount), 0)
      : events.length;
    return { events, totals: statsSummary(totals) };
  };
  ...
  const buildSignals = (evidenceRaw, settingsRaw) => {
    ...
    if (
      ...
      signals.push({
        id: ["regular_performance", regularRevision, performanceRegular.countedAttacks, performanceRegular.starsTotal, performanceRegular.totalDestruction].join(":"),
        reasonCode: "regular_performance",
        title: "Regular-war results",
        text: formatNumber(performanceRegular.averageStars, 1) + " stars · " +
          formatNumber(performanceRegular.averageDestruction, 0) + "% · " +
          performanceRegular.countedAttacks + " counted " + plural(performanceRegular.countedAttacks, "attack") +
    ...
    if (
      ...
      signals.push({
        id: ["regular_mirror_pattern", regularRevision, contextAnalysis.mirrorEvaluableWars, contextAnalysis.mirrorViolationWars].join(":"),
    ...
    if (
      ...
      signals.push({
        id: ["regular_timing_pattern", regularRevision, contextAnalysis.timingEvaluableWars, contextAnalysis.lateLowTownHallWars, contextAnalysis.lateForcedHardWars].join(":"),
        reasonCode: "regular_timing_pattern",
        title: "Repeated late low-TH attacks",
        text: contextAnalysis.lateLowTownHallWars + " of " + contextAnalysis.timingEvaluableWars + " evaluable wars started after 70% of clan attacks; " +
          contextAnalysis.lateForcedHardWars + " ended in forced hard targets",
      });
    }
  ...
  const buildEvidenceAfter = (evidenceRaw, timestampRaw, baselineEvidenceRaw) => {
    ...
    const newestBaselineSeasonMs = Math.max(0, ...Array.from(baselineCwlById.values()).map((event) =>
      parseMs(event && event.seasonStartedAt) || parseMs(cwlSeasonStartAt(
        toText(event && event.label || event && event.id).replace(/^cwl:/, "").split(":after-close:")[0]
      ))
    ));
  ...
  const evidenceSentence = (reasonCode, evidenceRaw, settingsRaw) => {
    const evidence = evidenceRaw && typeof evidenceRaw === "object" ? evidenceRaw : {};
    const regular = statsSummary(evidence.regular);
    const cwl = statsSummary(evidence.cwl);
    const settings = sanitizeSettings(settingsRaw);
    ...
    if (reasonCode === "regular_performance" && performanceRegular.countedAttacks > 0) {
      return "Across " + performanceRegular.countedAttacks + " counted regular-war attacks" +
        (contextAnalysis.forcedHardAttackCount && performanceRegular !== regular
          ...
          : "") +
        ", the average result was " + formatNumber(performanceRegular.averageStars, 1) + " stars and " +
        formatNumber(performanceRegular.averageDestruction, 0) + "% destruction.";
    }
    if (reasonCode === "regular_mirror_pattern" && contextAnalysis.mirrorEvaluableWars > 0) {
      return "Across " + contextAnalysis.mirrorEvaluableWars + " recent wars where the mirror was still open, the first attack went elsewhere " +
        contextAnalysis.mirrorViolationWars + " times.";
    }
    if (reasonCode === "regular_timing_pattern" && contextAnalysis.timingEvaluableWars > 0) {
      return "As a lower-TH lineup member, the first attack came after 70% of clan attacks in " +
        contextAnalysis.lateLowTownHallWars + " of " + contextAnalysis.timingEvaluableWars + " recent evaluable wars; " +
        contextAnalysis.lateForcedHardWars + " of those led to a forced hard target.";
    }
  ...
  const createController = (optionsRaw) => {
    ...
    const mutate = (itemRaw, actionRaw, patchRaw, behaviorRaw) => {
      ...
      const closesEvidence = ["dismiss", "resolve", "approve_return", "close", "cancel_removal", "approve_rejoin"].includes(action);
      const patch = Object.assign(
        {},
        patchRaw && typeof patchRaw === "object" ? patchRaw : {},
        closesEvidence && item.currentEvidence ? { evidence: item.currentEvidence } : {}
      );
    ...
    const eventRow = (event, kind) => {
      ...
      if (kind === "regular") {
        for (const line of formatRegularAttackContextLines(event && event.context)) {
          main.appendChild(createElement("span", "wfu-event-row__meta", line));
        }
      }
    ...
    const evidenceSection = (item) => {
      ...
      section.appendChild(grid);
      const contextAnalysis = analyzeRegularContext(evidence.regularEvents, evidence.regular);
      if (contextAnalysis.contextualWarCount > 0) {
        const mode = sanitizeSettings(state.work.settings).regularContextMode;
        const modeText = {
          off: "Context is visible below but does not affect review rules.",
          explain: "Explain-only mode: context is shown to moderators while existing case rules stay unchanged.",
          assist: "Assist mode: validated forced hard targets are excluded from regular-war performance checks.",
          automatic: "Automatic mode: assist adjustments and conservative repeated-pattern gates are active.",
        }[mode];
        const details = [
          "Attack context reconstructed for " + contextAnalysis.contextualWarCount + " of " +
            (Array.isArray(evidence.regularEvents) ? evidence.regularEvents.length : 0) + " retained regular " +
            plural(Array.isArray(evidence.regularEvents) ? evidence.regularEvents.length : 0, "war") + ".",
          contextAnalysis.mirrorEvaluableWars
            ? contextAnalysis.mirrorViolationWars + "/" + contextAnalysis.mirrorEvaluableWars + " first attacks went away from an open mirror."
            : "",
          contextAnalysis.timingEvaluableWars
            ? contextAnalysis.lateLowTownHallWars + "/" + contextAnalysis.timingEvaluableWars + " lower-TH first attacks were late."
            : "",
          contextAnalysis.forcedHardAttackCount
            ? contextAnalysis.forcedHardAttackCount + " validated forced hard " + plural(contextAnalysis.forcedHardAttackCount, "target") + "."
            : "",
          modeText,
        ].filter(Boolean);
        section.appendChild(createElement("div", "wfu-state-callout", details.join(" ")));
      }
      const events = []
    ...
    const renderSettingsModal = (mount, previousModalRaw) => {
      ...
      const regContext = createSelect("wfu-select");
      addOption(regContext, "off", "Off · existing rules only", settings.regularContextMode === "off");
      addOption(regContext, "explain", "Explain only · recommended", settings.regularContextMode === "explain");
      addOption(regContext, "assist", "Assist · discount forced hard targets", settings.regularContextMode === "assist");
      addOption(regContext, "automatic", "Automatic · add repeated-pattern cases", settings.regularContextMode === "automatic");
      ...
      setControlKey(regContext, "rules-regular-context");
      appendChildren(regularGrid, [
        ...
        setField("Mirror and timing context", regContext),
      ]);
      ...
      regular.appendChild(createElement(
        "div",
        "wfu-closed-copy",
        "Explain is the safe default. Assist excludes only fully validated forced hit-ups from performance. Automatic also requires at least 5 evaluable wars, 3 incidents and a 60% pattern; late-attacker cases additionally require 2 forced hard targets.",
      ));
      ...
      form.addEventListener("submit", (event) => {
        ...
        const next = {
          ...
          regularContextMode: regContext.value,
